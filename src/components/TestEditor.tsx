...
export function TestEditor() {
  ...
  const handleRun = async () => {
    if (!selectedTest) return;
    await handleSave();
    const fresh = { ...selectedTest, name, description, script, variables: Object.fromEntries(variables.filter(v => v.key.trim()).map(v => [v.key.trim(), v.value])) };
    await runTest(fresh);
  };
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div>
          <label className="block text-xs font-medium text-slate-400 mb-1">
            Test Script
            <span className="ml-2 text-slate-500 font-normal">(plain English instructions for the AI)</span>
          </label>
          <textarea
            value={script}
          ...
          <p className="text-xs text-slate-500 mt-1">
            Use <code className="text-violet-400">{'${varName}'}</code> to reference variables.
          </p>
        </div>
        ...
        <div>
          ...
          {variables.length === 0 ? (
            <p className="text-xs text-slate-600 italic">No variables defined.</p>
          ) : (
